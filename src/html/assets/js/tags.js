async function get_authors(){
    $.ajax({
        url: 'http://localhost:8000/api/vendor/tags',
        type: 'GET',
        dataType: 'json',
        data: '',
        success(data) {
            ...
            window.pagObj = $('#pagination').twbsPagination({
                ...
                onPageClick: function (event, page) {
                    ...
                    $('#author').empty();
                    $.getJSON(
                        'http://localhost:8000/api/tag?s=' + (page-1)*12,
                        function (characters) {
                            characters.forEach(function (character) {
                                if (character.role == 1) {
                                    let tags = $(`
                                   <div class="card mb-3 me-2 author" style="max-height: 4rem; min-width: 49%"  id="${character.tag_title}">
                                        <div class="card-body py-3">
                                            <div class="d-flex justify-content-between" >
                                                <div class="d-flex align-items-center">
                                                    <h5 class="card-title">${character.tag_title}</h5>
                                                </div>
                            ...
                            $('#author .author').each(function () {
                                ...
                                $(btn_del).click(function () {
                                    $('#accept-btn').click(function () {
                                        $.ajax({
                                            url: 'http://localhost:8000/api/tag/' + author_id,
                                            type: 'DELETE',
                                            success() {
                                                location.reload();
                                            },
